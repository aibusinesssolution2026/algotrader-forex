...
def test_win_loss_ratio_none_before_any_closed_trade():
    """The exact case that prompted this fix: 0 closed trades must render
    as undefined, never as a real 0.00 win rate."""
    ex, pf, *_ = make_stack()
    tracker = MetricsTracker(ex)
    m = tracker.snapshot()
    assert m.win_loss_ratio is None
    assert m.win_count == 0 and m.loss_count == 0
    assert m.n_trades == 0


def test_win_loss_ratio_real_zero_after_stop_loss_closes():
    """Two genuine stop-loss closes (both losses) must show a REAL 0.0,
    distinguishable from the None/undefined case above."""
    ex, pf, *_ = make_stack()
    tracker = MetricsTracker(ex)
    ...
    ex.process(SignalEvent("AAPL", side=1, price=150.0, atr=3.0))
    stop = pf.positions["AAPL"].stop_price
    ex.process(PriceEvent("AAPL", stop - 0.01))   # stop-loss close (a loss)

    m = tracker.snapshot()
    assert m.win_loss_ratio == 0.0            # real zero now, not None
    assert m.win_count == 0
    assert m.loss_count == 1
    assert m.n_trades == 1


def test_win_loss_ratio_with_a_real_win(monkeypatch):
    """A closed trade with positive PnL must count as a win and produce a
    proper ratio, distinct from both cases above."""
    ex, pf, *_ = make_stack()
    tracker = MetricsTracker(ex)
    ex.closed_pnls.append(42.0)     # simulate one winning close
    m = tracker.snapshot()
    assert m.win_loss_ratio == 1.0
    assert m.win_count == 1 and m.loss_count == 0
