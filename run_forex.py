...
def _fetch_with_retry(pairs, period, offline, max_attempts: int = 6,
                      base_delay: float = 10.0):
    """Startup data fetch MUST succeed before the loop can begin (models
    need data to train on) — but a container restarting into a transient
    Yahoo block should retry with backoff, not crash and burn a Railway
    restart attempt for nothing."""
    for attempt in range(1, max_attempts + 1):
        try:
            return get_fx_data(pairs, period, offline)
        except Exception as exc:
            if attempt == max_attempts:
                log.critical("Startup data fetch failed %d times, giving "
                            "up: %s", max_attempts, exc)
                raise
            delay = min(base_delay * attempt, 120)
            log.warning("Startup fetch attempt %d/%d failed (%s); "
                       "retrying in %.0fs", attempt, max_attempts, exc, delay)
            time.sleep(delay if not offline else 0.1)
...
def _build_position_details(portfolio, current_prices: dict,
                            open_decisions: dict) -> list[dict]:
    """One row per open position: entry, stop, live price, unrealized P&L,
    and which model tier decided to open it (honesty over impressiveness —
    shows 'statistical' or 'macd', whichever genuinely fired)."""
    out = []
    for sym, pos in portfolio.positions.items():
        price = current_prices.get(sym, pos.entry_price)
        unrealized = pos.unrealized(price)
        risk_amt = pos.open_risk()
        out.append({
            "symbol": sym,
            ...
            "decided_by": open_decisions.get(sym, "unknown"),
        })
    return out
...
def cmd_deploy_web(args):
    """Same continuous paper loop as `deploy`, PLUS a mobile-friendly web
    dashboard served from this same process (no second Railway service,
    no shared volume — state lives in memory, one thread trades, one
    thread serves http). View at https://<your-app>.up.railway.app/"""
    import threading
    import uvicorn
    from algotrader.execution.executor import PriceEvent, SignalEvent
    from algotrader.dashboard.metrics import MetricsTracker
    from algotrader.dashboard.terminal import render
    from algotrader.dashboard.live_state import LiveState
    from algotrader.dashboard.web import create_dashboard_app
    from rich.console import Console

    cfg = AppConfig()
    log.info("Trading mode: %s (paper=%s)", cfg.trading_mode, cfg.is_paper)
    data = _fetch_with_retry(args.pairs, args.period, args.offline)
    feats = {s: engineer_features(clean(df)) for s, df in data.items()}
    models = build_models(feats, train_frac=1.0)
    ex, pf, risk, _ = _make_stack(cfg, list(feats))
    tracker, console = MetricsTracker(ex), Console()
    ens = EnsemblePredictor(primary=None)
    state = LiveState()

    # web server in a background thread; trading loop stays on main thread
    app = create_dashboard_app(state, pairs=list(feats), offline=args.offline)
    server_thread = threading.Thread(
        target=lambda: uvicorn.run(app, host="0.0.0.0", port=args.port,
                                   log_level="warning"),
        daemon=True, name="dashboard")
    server_thread.start()
    log.info("Dashboard listening on :%d — open it in your browser.",
            args.port)
    ...
    seen_bars: dict[str, object] = {}
    open_decisions: dict[str, str] = {}   # symbol -> which model tier opened it
    consecutive_errors = 0
    try:
        while running["on"] and not risk.halted:
            if not market_open() and not args.offline:
                state.log_event("Market closed (weekend); waiting.")
                time.sleep(POLL_SECONDS)
                continue
            try:
                fresh = get_fx_data(args.pairs, "5d", args.offline)
                for sym, raw in fresh.items():
                    ...
                    if seen_bars.get(sym) == row.name:
                        ...
                        continue
                    seen_bars[sym] = row.name
                    before_positions = set(pf.positions)
                    ex.process(PriceEvent(sym, float(row["close"]),
                                          ts=row.name.to_pydatetime()))
                    if risk.halted:
                        state.log_event("CIRCUIT BREAKER TRIPPED — flattening "
                                        "and halting.")
                        break
                    closed = before_positions - set(pf.positions)
                    for c in closed:
                        state.log_event(f"{c}: stop-loss closed position")
                    ens.fallback = models.get(sym)
                    tier_before = dict(ens.tier_counts)
                    sig = ens.predict(row)
                    tier_used = next((t for t in ("primary", "statistical",
                                                   "macd")
                                      if ens.tier_counts[t] > tier_before[t]),
                                     "macd")
                    if sig != 0 and sym not in pf.positions:
                        ex.process(SignalEvent(sym, sig, float(row["close"]),
                                               float(row["atr_14"]),
                        ...
                        if sym in pf.positions:
                            open_decisions[sym] = tier_used
                            state.log_event(
                                f"{sym}: opened {'long' if sig>0 else 'short'} "
                                f"@ {row['close']:.5f} (model: {tier_used})")
                current_prices = {s: ex.prices.get(s) for s in feats
                                  if ex.prices.get(s)}
                state.update_positions(_build_position_details(
                    pf, current_prices, open_decisions))
                consecutive_errors = 0
            except Exception as exc:
                # NEVER let a transient fetch/network error kill the process —
                # log it, surface it on the dashboard, back off, keep running.
                consecutive_errors += 1
                log.exception("Iteration failed (consecutive: %d): %s",
                             consecutive_errors, exc)
                state.log_event(
                    f"⚠ iteration error ({type(exc).__name__}): {exc}")
                backoff = min(POLL_SECONDS * consecutive_errors, 1800)
                time.sleep(backoff if not args.offline else 1)
                continue
            m = tracker.snapshot()
            state.update(m.__dict__)
            console.print(render(tracker))
            if risk.halted:
                log.critical("Circuit breaker: deployment halted.")
                break
        ...
        ex.stop()
    console.print(render(tracker))
    log.info("Deploy-web loop exited cleanly.")
...
def main(argv=None):
    ...
    for name, fn in (("backtest", cmd_backtest), ("paper", cmd_paper),
                     ("deploy", cmd_deploy), ("deploy-web", cmd_deploy_web),
                     ("mtf", cmd_mtf), ("sweep", cmd_sweep),
                     ("serve", cmd_serve)):
